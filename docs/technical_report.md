---
title: "Replicating a Spherical Fuzzy SWARA–EDAS Framework for Pedestrian Safety in Bus Rapid Transit Systems"
subtitle: "A From-Scratch Reproduction of Ayyildiz et al. (2026)"
author: "[Your Name] — MSc Intelligent Systems"
date: "2026-07-23"
...
Figure 2.1 illustrates the overall workflow implemented in this project.
<img src="technical_flowchart_diagram.png" alt="flow_chart" width="300">

### 2.1 Stage 1 — SF-SWARA
...
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `models.py`     | Defines the `SphericalFuzzyNumber` data model and core data structures used throughout the implementation.                                                                                                                 |
| `loader.py`     | Loads and parses the case-study datasets and linguistic evaluation tables from CSV files.                                                                                                                                  |
| `spherical_fuzzy.py`      | Implements spherical fuzzy number operations, score calculations, and supporting mathematical utilities.                                                                                                                   |
| `swam.py`       | Implements the Spherical Weighted Arithmetic Mean (SWAM) operator used to aggregate expert evaluations.                                                                                                                    |
| `sf_swara.py`   | Implements the complete SF-SWARA criterion-weighting procedure, including score computation, criterion ranking, comparative importance, coefficient calculation, recursive weight computation, and weight normalisation.   |
| `sf_edas.py`    | Implements the SF-EDAS strategy evaluation procedure, including decision matrix aggregation, average solution computation, PDA/NDA calculation, weighted normalisation, appraisal score computation, and strategy ranking. |
| `validation.py` | Compares computed intermediate and final results against the published values reported in the paper to assess implementation accuracy.                                                                                     |
| `main.py`       | Coordinates the execution of the complete pipeline and produces the final outputs.                                                                                                                                         |
Figure 3.1 Python scripts and brief description.
...
- SF-SWARA score values.
- Criterion ranking.
- Normalised criterion weights.
- Aggregated spherical fuzzy decision matrix.
- Positive Distance from Average (PDA) values.
- Negative Distance from Average (NDA) values.
- Final appraisal scores.
- Final strategy ranking.

Where published intermediate values were available, the implementation outputs were compared directly against the corresponding tables presented in the paper. Numerical comparisons were performed using appropriate tolerances to account for the rounding precision used in the published results.
Table 4.1 summarises the validation outcomes.
...
Figure 4.1 illustrates the component and validation outcome.
...
| Rank  | Strategy                                                                     |
| ----- | ---------------------------------------------------------------------------- |
| **1** | Awareness campaigns and fines for pedestrians who violate traffic rules (S2) |
| **2** | Relocating Bus Rapid Transit lanes off the roadway (S1)                      |
| **3** | Special design and operational considerations for vulnerable road users (S3) |
Figure 5.1 illustrates the component and validation outcome.

This ranking matches the ordering reported in the original paper.

### 5.3 Overall Reproduction
...
- The SF-SWARA score values were reproduced successfully.
- The criterion ranking matched the published results exactly.
- The final strategy ranking matched the published results exactly.
- The highest-priority pedestrian safety intervention remained awareness campaigns combined with enforcement measures.
...
### 7.2 Translating Mathematical Formulations into Code
...
### 7.3 Limited Implementation Detail
...
### 7.4 Validation of Intermediate Results
...
## 10. References
Ayyildiz, E., Bouraima, M. B., Kose, Y., Murat, M., Karaca, T. K., & Yalcin Kavus, B. (2026).
Decision-Making Framework for Pedestrian Safety Optimisation in Bus Rapid Transit Systems.
IET Intelligent Transport Systems, 20, e70263.
https://doi.org/10.1049/itr2.70263